...
class NotesFragmentAdapter(private var onListItemClickListener: OnListItemClickListener,
private var notesData: MutableList<NoteData>) : RecyclerView.Adapter<NotesFragmentAdapter.BaseViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder {
        val inflater  = LayoutInflater.from(parent.context)
        return if (viewType == SIMPLE_TYPE){
            SimpleNoteViewHolder(inflater.inflate(R.layout.notes_recycler_view_simple_item, parent, false) as View)
        } else { NoteWithDateViewHolder(inflater.inflate(R.layout.notes_recycler_view_item_with_data, parent, false) as View)
     }
    }
    ...
    inner class NoteWithDateViewHolder(view: View)  : BaseViewHolder(view){
        override fun bind(noteData: NoteData){
            (itemView.findViewById(R.id.note_creation_date) as TextView)?.text =  noteData.creationDate
        }
    }

    inner open class BaseViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        open fun bind(noteData: NoteData){
    ...
    private fun generateItem() = NoteData("NoteTittle ${notesData.size}", "NoteDescription ${notesData.size}")
